...
const GarageSaveForm: React.FC<Props> = ({ settings }) => {
    ...
    return (
        <div className="w-100 box bg-white p-0">
            <div className="box__header">Save for later</div>
            <div className="box__content">
                <form className="row" onSubmit={handleSubmit(onSubmit)}>
                    <div className="col-24 form-group">
                        <label htmlFor="emailSave">Email Address *</label>
                        <input type="email" className={`form-control ${errors.email ? "is-invalid" : ""}`} id="emailSave" name="email" aria-describedby="email" placeholder="E.g. johnsmith@website.co.uk"
                            maxLength={50}
                            ref={register({
                    ...
                    <div className="col-24">
                        <button type="submit" className="btn btn-outline-secondary btn-lg btn-fluid btn-block" disabled={settings.isDisabled}>
                            {isLoading ? (<FontAwesomeIcon width="16" height="16" icon={faSpinner} className="fa-spin" />) : "Save My Garage"}
                        </button>
                    </div>
                </form>
            </div>
